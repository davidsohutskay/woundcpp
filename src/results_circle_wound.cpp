...
//    readfile.close();
    ...
    for(int sample = 0; sample < 1; sample++){
        ...
        double tau_lamdaP_a = 0.001*0.05/(K_phi_rho+1); // time constant for direction a, on the order of a year
        double tau_lamdaP_s = 0.001*0.05/(K_phi_rho+1); // time constant for direction s, on the order of a year
        //
        // solution parameters
